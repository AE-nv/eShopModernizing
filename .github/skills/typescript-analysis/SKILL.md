...
When `ast-grep` is available, prefer it over text-based search for structural code discovery. Load the `ast-grep` skill for full reference.

### Quick TypeScript/JavaScript Patterns

```bash
# Discovery: find all React hooks usage
ast-grep run -p 'useState($$$)' -l tsx --json
ast-grep run -p 'useEffect($$$)' -l tsx --json
ast-grep run -p 'useContext($CTX)' -l tsx --json
ast-grep run -p 'useCallback($$$)' -l tsx --json
ast-grep run -p 'useMemo($$$)' -l tsx --json

# Discovery: find NestJS decorators
ast-grep run -p '@Controller($$$)' -l typescript -C 3 --json
ast-grep run -p '@Injectable()' -l typescript -C 3 --json
ast-grep run -p '@Module($$$)' -l typescript --json

# Discovery: find Express routes
ast-grep run -p '$APP.get($PATH, $$$)' -l typescript --json
ast-grep run -p '$APP.post($PATH, $$$)' -l typescript --json
ast-grep run -p '$ROUTER.get($PATH, $$$)' -l typescript --json
ast-grep run -p '$ROUTER.post($PATH, $$$)' -l typescript --json

# Flow tracing: find all async functions
ast-grep run -p 'async function $NAME($$$) { $$$ }' -l typescript --json
ast-grep run -p 'async ($$$) => { $$$ }' -l typescript --json

# Flow tracing: find fetch/axios calls
ast-grep run -p 'await fetch($$$)' -l typescript --json
ast-grep run -p 'await axios.$METHOD($$$)' -l typescript --json

# Data access: find TypeORM/Prisma operations
ast-grep run -p 'this.repository.$METHOD($$$)' -l typescript --json
ast-grep run -p 'this.prisma.$MODEL.$METHOD($$$)' -l typescript --json

# Exports: find all exported declarations
ast-grep run -p 'export function $NAME($$$) { $$$ }' -l typescript --json
ast-grep run -p 'export class $NAME { $$$ }' -l typescript --json
ast-grep run -p 'export interface $NAME { $$$ }' -l typescript --json

# Error handling: find try/catch blocks
ast-grep run -p 'try { $$$ } catch ($E) { $$$ }' -l typescript --json
```

### TypeScript Rule File Example

```yaml
# find-components-with-side-effects.yml
id: components-with-side-effects
language: TSX
rule:
  all:
