...
ast-grep run -p '@PutMapping($$$)' -l java --json
ast-grep run -p '@DeleteMapping($$$)' -l java --json
ast-grep run -p '@RequestMapping($$$)' -l java --json

# Discovery: find service/repository/component classes
ast-grep run -p '@Service' -l java -C 3 --json
ast-grep run -p '@Repository' -l java -C 3 --json
ast-grep run -p '@Component' -l java -C 3 --json

# Flow tracing: find injected dependencies
ast-grep run -p '@Autowired' -l java -C 2 --json

# Data access: find custom queries
ast-grep run -p '@Query($QUERY)' -l java --json

# Business rules: find validation annotations
ast-grep run -p '@Valid' -l java --json
ast-grep run -p '@NotNull($$$)' -l java --json

# Integration: find scheduled jobs and listeners
ast-grep run -p '@Scheduled($$$)' -l java --json
ast-grep run -p '@KafkaListener($$$)' -l java --json
ast-grep run -p '@RabbitListener($$$)' -l java --json

# Transaction boundaries
ast-grep run -p '@Transactional' -l java -C 3 --json
ast-grep run -p '@Transactional($$$)' -l java --json
```

### Java Rule File Example

```yaml
# find-services-with-external-calls.yml
id: services-with-external-calls
language: Java
rule:
  all:
    ...
    - inside:
        ...
        stopBy: end
    - has:
        regex: "Client|Template|WebClient"
        stopBy: end
```
