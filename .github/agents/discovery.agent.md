...
## Skill Routing
- `cobol` → `cobol-analysis` + `ast-grep`
- `java` → `java-analysis` + `ast-grep`
- `typescript` or `javascript` → `typescript-analysis` + `ast-grep`
- `vbnet` (FRUTAS) → `vbnet-analysis`
- `python` → `python-analysis` (if available) + `ast-grep`
- `dotnet` → `dotnet-analysis` (if available) + `ast-grep`

> **ast-grep**: For languages with tree-sitter support, also load the `ast-grep` skill for deterministic structural search. Use `ast-grep run -p '<pattern>' -l <lang> --json` to find entry points, annotations, and code structures with precision instead of text-based grep.
