...
|----------|-------------------|-------|
| COBOL | `.cbl`, `.cob`, `.cpy`, JCL | `cobol-analysis` + `ast-grep` |
| Java | `.java`, `pom.xml`, `build.gradle` | `java-analysis` + `ast-grep` |
| TypeScript | `.ts`, `tsconfig.json`, `package.json` | `typescript-analysis` + `ast-grep` |
| VB.NET WinForms | `.vb`, `.sln`, `SARB_DA`, `SARB_DATA` | `vbnet-analysis` |
| Python | `.py`, `requirements.txt`, `pyproject.toml` | `python-analysis` + `ast-grep` |
| C#/.NET | `.cs`, `.csproj`, `.sln` | `dotnet-analysis` + `ast-grep` |

> **ast-grep skill**: Cross-cutting deterministic search tool for languages with tree-sitter support (Java, TypeScript, JavaScript, C#, Python, Go, Kotlin, etc.) and custom languages (COBOL via tree-sitter-cobol). Agents should load this skill alongside the language skill when the target language is supported. Not available for VB.NET.
